    ...
    return (
      <>
        <div className={ ( menu && window.innerWidth<500 ? 'h-36': 'h-16') + " md:h-full transition-all duration-300 nav z-[10] m-[0.1rem] rounded p-2 flex md:justify-center md:content-center "+navCol} >
            ...
            {
              menu &&
              <div className={ "options md:w-1/3 w-[50%] md:me-[0%] mr-[40%] flex md:flex-row  flex-col m-2  content-center text-sm font-semibold text-white justify-evenly"}>
            ...
            <div className="other text-sm md:relative fixed top-[-10%] w-1/3 grid grid-flow-col justify-items-end">
                  {
                    personState != '' &&
                    <>
                      <a onClick={()=>{localStorage.removeItem('mytoken');setPersonState('')}}  className="text-slate-600 bg-slate-100 px-1 py-2 border border-red-600 opacity-20 hover:opacity-100 hover:bg-red-100 hover:text-red-600 rounded-xl">Sign Out</a>
                    </>
                  }
            </div>
        </div>
